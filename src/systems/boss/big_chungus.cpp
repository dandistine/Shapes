...
BigChungusLeadInSystem::BigChungusLeadInSystem(int power, entt::dispatcher& dispatcher, entt::entity player, entt::registry& reg, olc::PixelGameEngine* pge) : power(power), dispatcher(dispatcher), player_entity(player), System(reg, pge) {};

void BigChungusLeadInSystem::OnUserUpdate(float fElapsedTime) {
...
BigChungusBossSystem::BigChungusBossSystem(int power, entt::dispatcher& dispatcher, entt::entity player, entt::registry& reg, olc::PixelGameEngine* pge) : power(power), dispatcher(dispatcher), player_entity(player), System(reg, pge) {
    dispatcher.sink<BeginBossMain>().connect<&BigChungusBossSystem::on_boss_main>(this);
};

BigChungusBossSystem::~BigChungusBossSystem() {
    dispatcher.sink<BeginBossMain>().disconnect(this);
}

void BigChungusBossSystem::on_boss_main(const BeginBossMain& boss) {
...
BigChungusLeadOutSystem::BigChungusLeadOutSystem(int power, entt::dispatcher& dispatcher, entt::entity player, entt::registry& reg, olc::PixelGameEngine* pge) : power(power), dispatcher(dispatcher), player_entity(player), System(reg, pge) {};

void BigChungusLeadOutSystem::OnUserUpdate(float fElapsedTime) {
    total_time += fElapsedTime;
...
BigChungusFactory::BigChungusFactory(entt::dispatcher& dispatcher, entt::entity player, entt::registry& reg, olc::PixelGameEngine* pge) : dispatcher(dispatcher), player_entity(player), reg(reg), pge(pge) {};

std::unique_ptr<System> BigChungusFactory::GetLeadInSystem(int power) const {
    return std::make_unique<BigChungusLeadInSystem>(power, dispatcher, player_entity, reg, pge);
};

std::unique_ptr<System> BigChungusFactory::GetBossSystem(int power) const {
    return std::make_unique<BigChungusBossSystem>(power, dispatcher, player_entity, reg, pge);
};

std::unique_ptr<System> BigChungusFactory::GetLeadOutSystem(int power) const {
